...
export function AllJobs() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return result;
  }, [allJobs, searchTerm, sortConfig, filterConfig]);
  ...
  const handleQuickFilter = (type: 'supplier' | 'stock', value: string) => {
    ...
    const tableElement = document.getElementById('all-jobs-table');
    if (tableElement) {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, 'Tum_Isler.xlsx');
    setIsExportMenuOpen(false);
  };

  const exportToPDF = () => {
    const doc = new jsPDF();
    const tableColumn = ["İş Emri No", "Tedarikçi", "Miktar", "Tarih", "Durum"];
    ...
    doc.text("Tüm İşler Arşivi", 14, 15);
    doc.save("Tum_Isler.pdf");
    setIsExportMenuOpen(false);
  };
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 shrink-0 border-b border-border bg-bg-card/30 backdrop-blur-md">
        <div>
          <h1 className="text-xl font-display font-black tracking-tight text-text-primary">Tüm İşler</h1>
          <p className="text-[10px] text-text-secondary font-bold uppercase tracking-widest opacity-70">İş Emirleri Arşivi</p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden bg-bg-app/20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 p-6 shrink-0">
          {[
            ...
            { 
              title: 'Tamamlanan', 
              value: allJobs.filter(j => j.status === 'Tamamlandı').length.toString(), 
              icon: <CheckCircle2 size={16} />, 
              color: 'text-blue-500',
              bg: 'bg-blue-500/10'
            },
          ...
          >
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
                    ...
                    >
                      ...
                      <td className="p-4" onClick={() => handleEditJob(job)}>
                        <div className="flex items-center gap-3">
                          <div className={`w-1.5 h-1.5 rounded-full ${job.status === 'Tamamlandı' ? 'bg-emerald-500' : 'bg-blue-500'} shadow-[0_0_8px_rgba(0,0,0,0.1)]`} />
                      ...
                      <td className="p-4 text-right font-mono font-black text-text-primary text-lg" onClick={() => handleEditJob(job)}>
                        {job.items.reduce((sum: number, item: any) => sum + item.qty, 0).toLocaleString()}
                      </td>
                      <td className="p-4" onClick={() => handleEditJob(job)}>
              ...
              {filteredJobs.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-text-secondary opacity-40">
                  ...
                  <p className="font-display font-bold">Arşivde kayıt bulunamadı</p>
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-border flex justify-between items-center bg-bg-card/30 shrink-0">
              <p className="text-[10px] font-bold uppercase tracking-widest text-text-secondary">
                Toplam <span className="text-text-primary">{filteredJobs.length}</span> Kayıt
              </p>
        ...
        {isFilterModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-bg-card border border-border rounded-[var(--radius-xl)] shadow-2xl overflow-hidden"
            >
              <div className="p-6 border-b border-border flex justify-between items-center bg-bg-app/30">
                ...
                <button onClick={() => setIsFilterModalOpen(false)} className="p-2 text-text-secondary hover:text-text-primary transition-colors rounded-full hover:bg-bg-app">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Tedarikçi</label>
                    <select 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Durum</label>
                    <select 
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Başlangıç Tarihi</label>
                    <input 
                      type="date" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Bitiş Tarihi</label>
                    <input 
                      type="date" 
              ...
              <div className="p-6 bg-bg-app/30 border-t border-border flex justify-between gap-4">
                <button 
