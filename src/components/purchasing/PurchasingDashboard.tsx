...
// Mock Data
const MOCK_STATS = [
  { id: 1, label: 'Aktif Talepler', value: '0', change: '0', trend: 'up', icon: <FileText size={20} />, color: 'text-focus-neon' },
  { id: 2, label: 'Bekleyen Onaylar', value: '0', change: '0', trend: 'down', icon: <CheckCircle2 size={20} />, color: 'text-nrg-sun' },
  { id: 3, label: 'Aylık Harcama', value: '₺0', change: '0%', trend: 'up', icon: <TrendingUp size={20} />, color: 'text-grow-main' },
  { id: 4, label: 'Tedarikçi Sayısı', value: '0', change: '0', trend: 'up', icon: <Users size={20} />, color: 'text-ai-bright' },
];

const MOCK_PURCHASES: any[] = [];

const SPENDING_DATA: any[] = [];
      ...
      >
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            >
              <Zap size={12} className="animate-pulse" /> Akıllı Satınalma Aktif
            </motion.div>
            <motion.div 
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              <ShieldCheck size={12} /> Bütçe Kontrolü
            </motion.div>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-text-secondary font-medium text-lg tracking-tight opacity-70">Satınalma operasyonel zekasını yönetin.</p>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-6 py-3 bg-focus-main text-pure-white rounded-2xl font-display font-black text-sm tracking-tighter hover:scale-105 active:scale-95 transition-all shadow-lg shadow-focus-main/20 group">
            <Plus size={18} className="group-hover:rotate-90 transition-transform duration-500" />
            YENİ TALEP OLUŞTUR
          </button>
          <button className="w-12 h-12 flex items-center justify-center bg-skel-matte/5 border border-skel-metal/10 rounded-2xl text-text-secondary hover:bg-skel-matte/10 hover:text-text-primary transition-all group" title="Tedarikçi Listesi">
            <Users size={20} className="group-hover:scale-110 transition-transform" />
          </button>
          <button className="w-12 h-12 flex items-center justify-center bg-skel-matte/5 border border-skel-metal/10 rounded-2xl text-text-secondary hover:bg-skel-matte/10 hover:text-text-primary transition-all group" title="Bütçe Analizi">
        ...
        >
          <div className="p-6 border-b border-skel-metal/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-display font-black text-text-primary tracking-tight">Aktif Satınalma Emirleri</h3>
                <p className="text-[10px] label-mono opacity-50 uppercase tracking-widest">Onay ve tedarik süreci takibi</p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-skel-matte/5 border-b border-skel-metal/5">
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Emir ID / Tedarikçi</th>
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Kategori</th>
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Tutar</th>
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Durum</th>
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Tarih</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-skel-metal/5">
                {MOCK_PURCHASES.map((purchase) => (
                  <tr key={purchase.id} className="hover:bg-skel-matte/5 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-xs font-mono text-focus-neon font-bold">{purchase.id}</span>
                        <span className="text-sm font-display font-bold text-text-primary tracking-tight">{purchase.supplier}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        <Tag size={12} className="text-skel-metal" />
                        <span className="text-xs font-medium text-text-secondary">{purchase.category}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm font-mono font-bold text-text-primary">{purchase.amount}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={clsx(
                        "px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter",
                        purchase.status === 'Tamamlandı' ? "bg-grow-main/10 text-grow-main" :
                        purchase.status === 'İptal Edildi' ? "bg-crit-blood/10 text-crit-vivid" :
                        purchase.status === 'Beklemede' ? "bg-nrg-sun/10 text-nrg-sun" :
                    ...
                    <td className="px-6 py-4">
                      <span className="text-[10px] font-bold text-text-secondary opacity-60">{purchase.date}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="p-4 border-t border-skel-metal/5 text-center">
            <button className="text-[10px] font-black text-focus-neon uppercase tracking-widest hover:underline">
              TÜM SATINALMA KAYITLARINI GÖRÜNTÜLE
            </button>
          </div>
        </motion.div>
        ...
        <div className="space-y-8">
          ...
          >
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-lg font-display font-black text-text-primary tracking-tight">Harcama Trendi</h3>
                <p className="text-[10px] label-mono opacity-50 uppercase tracking-widest">Aylık satınalma hacmi (bin ₺)</p>
              </div>
              <div className="text-grow-main flex items-center gap-1 text-[10px] font-black">
                <TrendingUp size={14} /> %15 Tasarruf
              </div>
            </div>
            ...
            <div className="h-48 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={SPENDING_DATA}>
                  ...
                  />
                  <Area type="monotone" dataKey="amount" stroke="#2ED573" strokeWidth={3} fillOpacity={1} fill="url(#colorSpend)" />
