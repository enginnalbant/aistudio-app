...
export const MobileNav = React.memo(function MobileNav({ activeModule, setActiveModule, toggleSidebar }: MobileNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-20 bg-skel-space/80 backdrop-blur-xl border-t border-skel-metal/10 z-50 lg:hidden px-4 flex items-center justify-between shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
      {navItems.map((item) => {
