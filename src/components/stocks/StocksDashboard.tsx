...
// Mock Data
const MOCK_STATS = [
  { id: 1, label: 'Toplam Kalem', value: '0', change: '0%', trend: 'up', icon: <Package size={20} />, color: 'text-focus-neon' },
  { id: 2, label: 'Kritik Seviye', value: '0', change: '0', trend: 'up', icon: <AlertTriangle size={20} />, color: 'text-nrg-sun' },
  { id: 3, label: 'Stokta Yok', value: '0', change: '0', trend: 'down', icon: <Activity size={20} />, color: 'text-crit-vivid' },
  { id: 4, label: 'Aylık Hareket', value: '0', change: '0%', trend: 'up', icon: <ArrowRightLeft size={20} />, color: 'text-grow-main' },
];

const MOCK_LOW_STOCK: any[] = [];
      ...
      >
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            >
              <RefreshCw size={12} className="animate-spin-slow" /> Envanter Güncel
            </motion.div>
            <motion.div 
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              <ShieldCheck size={12} /> Veri Güvenliği
            </motion.div>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-text-secondary font-medium text-lg tracking-tight opacity-70">Envanter operasyonel zekasını yönetin.</p>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        >
          <div className="p-6 border-b border-skel-metal/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-nrg-sun/10 flex items-center justify-center text-nrg-sun">
                <AlertTriangle size={20} />
              </div>
              <div>
                <h3 className="text-lg font-display font-black text-text-primary tracking-tight">Kritik Stok Uyarıları</h3>
                <p className="text-[10px] label-mono opacity-50 uppercase tracking-widest">Acil tedarik gerektiren kalemler</p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-skel-matte/5 border-b border-skel-metal/5">
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Stok Kodu / Adı</th>
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Kategori</th>
                  <th className="px-6 py-4 label-mono text-[10px] opacity-50 uppercase tracking-widest">Mevcut / Min</th>
              ...
              <tbody className="divide-y divide-skel-metal/5">
                {MOCK_LOW_STOCK.map((item) => (
                  <tr key={item.id} className="hover:bg-skel-matte/5 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-xs font-mono text-focus-neon font-bold">{item.id}</span>
                        <span className="text-sm font-display font-bold text-text-primary tracking-tight">{item.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className={clsx(
                          "text-xs font-bold",
                          item.current <= item.min / 2 ? "text-crit-vivid" : "text-nrg-sun"
                        )}>{item.current}</span>
                        <span className="text-[10px] text-text-secondary opacity-50">/ {item.min} {item.unit}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={clsx(
                        "px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter",
                        item.status === 'Yok' ? "bg-crit-blood/10 text-crit-vivid" :
                        item.status === 'Kritik' ? "bg-crit-vivid/10 text-crit-vivid" :
        ...
        <div className="space-y-8">
          ...
          >
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-lg font-display font-black text-text-primary tracking-tight">Stok Hareketleri</h3>
                <p className="text-[10px] label-mono opacity-50 uppercase tracking-widest">Giriş vs Çıkış Analizi</p>
              </div>
            ...
            <div className="h-48 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={MOVEMENT_DATA}>
                  ...
                  />
                  <Area type="monotone" dataKey="in" stroke="#70A1FF" fillOpacity={1} fill="url(#colorIn)" />
                  <Area type="monotone" dataKey="out" stroke="#FF4757" fillOpacity={1} fill="url(#colorOut)" />
            ...
            <div className="flex items-center justify-center gap-6 mt-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-text-secondary uppercase">Giriş</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-crit-vivid" />
                <span className="text-[10px] font-black text-text-secondary uppercase">Çıkış</span>
              </div>
            </div>
          </motion.div>
                ...
                <BarChart data={CATEGORY_DATA} layout="vertical">
                  ...
                  <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                    {CATEGORY_DATA.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={index % 2 === 0 ? '#70A1FF' : '#2ED573'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              ...
              {CATEGORY_DATA.map((item, idx) => (
                <div key={item.name} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 flex-1 mx-4">
                    <div className="flex-1 h-1 bg-skel-matte/10 rounded-full overflow-hidden">
                      <div className="h-full bg-focus-neon/40" style={{ width: `${item.value}%` }} />
                    </div>
                  </div>
